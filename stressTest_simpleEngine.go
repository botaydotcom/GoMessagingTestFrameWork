...
import (
	...
	"strconv"
	"sync"
	"xmltest/btalkTest/StressTestEngine2"
)
...
func mergeResult(clientResult ClientResult,
	...
	if clientResult.IsSuccessful {
		...
		reg2, _ := regexp.Compile("WSARecv tcp \\d{0,3}.\\d{0,3}.\\d{0,3}.\\d{0,3}:\\d{0,5}: An existing connection was forcibly closed by the remote host")
		matchResult = reg2.FindStringIndex(clientResult.Reason.Error())
		if matchResult != nil {
			message := clientResult.Reason.Error()[:matchResult[0]]
			fullMessage := message + "WSARecv tcp <address:port>: An existing connection was forcibly closed by the remote host"
			clientResult.Reason = errors.New(fullMessage)
		}

		reg3, _ := regexp.Compile("WSARecv tcp \\d{0,3}.\\d{0,3}.\\d{0,3}.\\d{0,3}:\\d{0,5}: The specified network name is no longer available.")
		matchResult = reg3.FindStringIndex(clientResult.Reason.Error())
		if matchResult != nil {
			message := clientResult.Reason.Error()[:matchResult[0]]
			fullMessage := message + "WSARecv tcp <address:port>: The specified network name is no longer available."
			clientResult.Reason = errors.New(fullMessage)
		}
