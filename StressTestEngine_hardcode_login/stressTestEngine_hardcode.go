package StressTestEngine_hardcode
...
func ExecuteTestSuite(addr *net.TCPAddr, testSuite *TestSuite) (TestResult){
	...
	var totalTime int64
	totalTime = 0
	userEmail := Helper.GetNextUserEmail()

	begin := time.Now()
	// send login message first
	sendLoginMsg(conn, userEmail)
	...
	totalTime += time.Now().Sub(begin).Nanoseconds()
	numRequest++

	// now send 60 hello message
	for {
		begin = time.Now()
		SpecialChannel <- 1
		sendHelloMsg(conn)
		_, err = readHelloReply(conn)
		totalTime += time.Now().Sub(begin).Nanoseconds()
...
func sendLoginMsg(conn *net.TCPConn, email string) {
	...
	data, err := proto.Marshal(loginInfo)
	if err != nil {
	...
	binary.Write(buf, binary.LittleEndian, byte(C2S_LoginInfo_CMD))
	if BYPASS_CONNECTION_SERVER {
		if DEBUG_SENDING_MESSAGE {
...
func readLoginReply(conn *net.TCPConn) (proto.Message, error) {
...
func sendHelloMsg(conn *net.TCPConn) {
	...
	data, err := proto.Marshal(helloInfo)
	if err != nil {
		log.Fatal("marchaling error: ", err)
	}
	length := int32(len(data)) + 1
